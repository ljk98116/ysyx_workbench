...
class PCReg extends Module
{
    ...
    var pc_reg = RegInit((base.RESET_VECTOR.U)(UInt(base.ADDR_WIDTH.W)))
    var inst_valid_mask = WireInit((0.U)(base.FETCH_WIDTH.W))
    switch(pc_reg(3, 0))
    {
        is(0.U){
            inst_valid_mask := 0b1111.U
        }
        is(4.U){
            inst_valid_mask := 0b0111.U
        }
        is(8.U){
            inst_valid_mask := 0b0011.U
        }
        is(12.U){
            inst_valid_mask := 0b0001.U
        }
    }
