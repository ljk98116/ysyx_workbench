...
class PCReg extends Module
{
    ...
    /* 更新GHR */
    GHR := (GHR << GHR_step) | Reverse(retire_br_taken_vec_mid.asUInt)
    var global_pht_idx_vec_o = WireInit(VecInit(
        Seq.fill(base.FETCH_WIDTH)((0.U)(base.PHTID_WIDTH.W))
    ))
    /* PC hash mapping */
    for(i <- 0 until base.FETCH_WIDTH){
        global_pht_idx_vec_o(i) := pc_reg(14, 2) ^ GHR
    }
    ...
    */
    ...
    var btb_idx_vec_o = WireInit(VecInit(
        Seq.fill(base.FETCH_WIDTH)((0.U)(base.PHTID_WIDTH.W))
    ))
    for(i <- 0 until base.FETCH_WIDTH){
    ...
    io.btb_idx_vec_o := btb_idx_vec_o
}