...
class ROB extends Module
{
    ...
    for(i <- 0 until base.ALU_NUM){
        when(io.cdb_i.alu_channel(i).valid & ~io.rat_flush_en & (i.U === io.cdb_i.alu_channel(i).rob_id(bankwidth + 1, bankwidth))
        ){
    ...
    for(i <- 0 until base.AGU_NUM){
        when(io.cdb_i.agu_channel(i).valid & ~io.rat_flush_en & (i.U === io.cdb_i.agu_channel(i).rob_id(bankwidth + 1, bankwidth))
        ){
