#include <isa.hpp>
#include <memory/host.hpp>
#include <memory/vaddr.hpp>
#include <device/map.hpp>
#include <device/zio.hpp>
#include <common.hpp>

namespace npc {
#define IO_SPACE_MAX (32 * 1024 * 1024)

static uint8_t *io_space = NULL;
static uint8_t *p_space = NULL;

uint8_t* new_space(int size) {
  uint8_t *p = p_space;
  // page aligned;
  size = (size + (PAGE_SIZE - 1)) & ~PAGE_MASK;
  p_space += size;
  assert(p_space - io_space < IO_SPACE_MAX);
  return p;
}

static void check_bound(IOMap *map, paddr_t addr) {
  if (map == NULL) {
    NPCAssert(map != NULL, "address (" NPC_FMT_PADDR ") is out of bound at pc = " NPC_FMT_WORD, addr, cpu.pc);
  } else {
    NPCAssert(addr <= map->high && addr >= map->low,
        "address (" NPC_FMT_PADDR ") is out of bound {%s} [" NPC_FMT_PADDR ", " NPC_FMT_PADDR "] at pc = " NPC_FMT_WORD,
        addr, map->name, map->low, map->high, cpu.pc);
  }
}

static void invoke_callback(io_callback_t c, paddr_t offset, int len, bool is_write) {
  if (c != NULL) { c(offset, len, is_write); }
}

void init_map() {
  io_space = (uint8_t*)malloc(IO_SPACE_MAX);
  assert(io_space);
  p_space = io_space;
}

word_t map_read(paddr_t addr, int len, IOMap *map) {
  assert(len >= 1 && len <= 8);
  check_bound(map, addr);
  paddr_t offset = addr - map->low;
  invoke_callback(map->callback, offset, len, false); // prepare data to read
  word_t ret = host_read(map->space + offset, len);
#ifdef CONFIG_DTRACE
  char buf[1024];
  memset(buf, 0, sizeof(buf));
  sprintf(buf, "DEVICE READ ADDR: 0x%x, len:%d, map: %s, data: 0x%x\n", addr, len, map->name, ret);
  iringbuf_write(dtrace_buf, buf, strlen(buf));
#endif
  return ret;
}

void map_write(paddr_t addr, int len, word_t data, IOMap *map) {
  assert(len >= 1 && len <= 8);
  check_bound(map, addr);
  paddr_t offset = addr - map->low;
  host_write(map->space + offset, len, data);
  invoke_callback(map->callback, offset, len, true);
#ifdef CONFIG_DTRACE
  char buf[1024];
  memset(buf, 0, sizeof(buf));
  sprintf(buf, "DEVICE WRITE ADDR: 0x%x, len:%d, map: %s, data: 0x%x\n", addr, len, map->name, data);
  iringbuf_write(dtrace_buf, buf, strlen(buf));
#endif
}
}