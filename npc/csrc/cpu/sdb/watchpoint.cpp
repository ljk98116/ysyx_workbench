...
WP *new_WP(){
    /* 无空闲 */
    if(!free_){
        NPCLog("No free watchpoint here");
        assert(0);
        return NULL;
    }
...
void free_WP(WP *wp){
    if(!wp){
        NPCLog("Error: try to free nullptr");
        assert(0);
        return;
    }
...
int DeleteWP(int NO){
    ...
    while(cur != NULL){
        if(cur->NO == NO){
            ...
            /* 释放该WP */
            free_WP(cur);
            NPCLog("watchpoint %d deleted successfully", NO);
            return found;
        }
        prev = cur;
        cur = cur->next;
    }
    NPCLog("watchpoint %d not found", NO);
    return found;
}
...
void watchpoint_step(){
    ...
    while(cur != NULL){
        ...
        if(!success){
            NPCLog("Error: watchpoint %d calculate expr: %s failed", cur->NO, cur->expr);
            assert(0);
        }
